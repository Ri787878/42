...
from models.drones import Drone, ActiveDrone
from models.zone_network import Hub, Zone_Network
            ...
            if len(conn_list) >= 2:
                ...
                if {left, right} == {hub_a, hub_b}:
                    ...
                    for elem in conn_list[2:]:
                        if isinstance(elem,
                                      dict) and "max_link_capacity" in elem:
        ...
        for attr_name in (
            ...
            if hasattr(network, attr_name):
                ...
                if isinstance(props, dict):
                    val = props.get((hub_a,
                                     hub_b)) or props.get((hub_b, hub_a))
                    if val is not None:
                        if isinstance(val,
                                      dict) and "max_link_capacity" in val:
    ...
    def simulate_drones(
        ...
        while active_drones or queued_drones:
            ...
            while (
                ...
                occupancy[start_hub.name] = (
                    occupancy.get(start_hub.name, 0) + 1
                )
            ...
            # Track active link usage to enforce
            # max_link_capacity during this turn
            ...
            for item in active_drones:
                ...
                if next_hub.name != goal_hub.name:
                    if not self._hub_has_capacity(next_hub, next_occupancy):
                        continue
                ...
                if next_hub.name != goal_hub.name:
                    occupancy[next_hub.name] = (
                        occupancy.get(next_hub.name, 0) + 1
                    )

                movement_name = self._movement_label(
                    network,
                    current_hub,
                    next_hub
                    )
            ...
            active_drones = [
                    item for item in active_drones
            ...
            if (active_drones or queued_drones) and not any_movement_occurred:
                stall_msg: str = "[WARNING] Simulation stalled! "
                stall_msg += "No drones can progress."
                print(stall_msg)
